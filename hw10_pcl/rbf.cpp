...
pcl::PolygonMesh rbf(pcl::PointCloud<pcl::PointNormal>::Ptr cloud_with_normals);

int main() {
    auto cloud = read_obj_and_convert_to_pcd("model/Kitten_04.obj", "model/Kitten_04.pcd");
    auto normals = compute_normal(cloud);
    pcl::PointCloud<pcl::PointNormal>::Ptr cloud_with_normals(new pcl::PointCloud<pcl::PointNormal>);
    pcl::concatenateFields(*cloud, *normals, *cloud_with_normals);

    auto mesh = rbf(cloud_with_normals);
    pcl::io::saveOBJFile(std::string(project_path) + "model/output_Kitten_04.obj", mesh);
}
...
pcl::PolygonMesh rbf(pcl::PointCloud<pcl::PointNormal>::Ptr cloud_with_normals) {
    ...
    rbf.setSearchMethod(kdt);
    rbf.setInputCloud(cloud_with_normals);
    rbf.setIsoLevel(0.0f);
    rbf.setGridResolution(50, 50, 50);
