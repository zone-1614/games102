...
MathUtil::ParameterizationUniform(const std::vector<float> &pos_x, const std::vector<float> &pos_y) {
    ...
    float step = 1.0f / (n - 1); //均匀参数化的步长
    std::vector<float> result(n);
    result[0] = 0.0f;
    for (size_t i = 1; i < n; i++) {
        result[i] = result[i - 1] + step; // 每次都增加步长
    }
    return result;
}

// 弧长参数化, 根据两点间的距离占总长度的多少来参数化
std::vector<float>
MathUtil::ParameterizationChoral(const std::vector<float> &pos_x, const std::vector<float> &pos_y) {
    ...
    float sum = 0.0f; // 总的长度
    std::vector<float> result(n); // 参数化的结果
    std::vector<float> dist(n - 1); // 相邻两点的距离

    // 计算dist
    for (size_t i = 1; i < n; i++) {
        dist[i - 1] = (Eigen::Vector2f(pos_x[i-1], pos_y[i-1]) - Eigen::Vector2f(pos_x[i], pos_y[i])).norm(); // 两点间距离
        sum += dist[i - 1];
    }
    ...
    for (size_t i = 1; i < n - 1; i++) {
        float step = dist[i - 1] / sum; // 占总距离的比例
